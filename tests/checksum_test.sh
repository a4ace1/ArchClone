...
SANDBOX="$(mktemp -d "${TMPDIR:-/tmp}/archclone-checksum-test.XXXXXX")"
...
if verify_checksums "$TEST" 2>/tmp/archclone-checksum-test-tamper.log; then
    echo "FAIL: verify_checksums did not detect tampering"
    exit 1
fi
echo "OK: verify_checksums correctly detected tampering"
rm -f /tmp/archclone-checksum-test-tamper.log
